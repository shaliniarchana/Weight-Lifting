...
export default function WomenCategoriesPage() {
  ...
  useEffect(() => {
    async function initCategories() {
      try {
        ...
        const categoryRef = collection(db, "matches", matchId, gender, genderDocId, "categories");
        ...
        if (existingNames.size >= 8) {
          localStorage.setItem(`categoryDocIds_${matchId}_${gender}`, JSON.stringify(categoryMap));
  ...
  async function removeSharedGenderDocId(matchId) {
    ...
    const menIds = new Set(menSnap.docs.map((doc) => doc.id));

    for (const docSnap of womenSnap.docs) {
      if (menIds.has(docSnap.id)) {
        await deleteDoc(doc(db, "matches", matchId, "women", docSnap.id));
        console.log(`🗑️ Removed duplicate gender doc ID '${docSnap.id}' from 'women'`);
      }
    }
  }
